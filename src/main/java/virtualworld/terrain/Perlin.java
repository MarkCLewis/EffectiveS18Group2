...
public class Perlin { //singleton class for noise function
	...
	public double noise3D(double x, double y, double z) {
		...
		//Finds a unit cube based off of the input parameters 
		int X = (int)Math.floor(x) & 255; 		
		int Y = (int)Math.floor(y) & 255;
		int Z = (int)Math.floor(z) & 255;
		
		//Find point on the unit cube
		x -= Math.floor(x);
		y -= Math.floor(y);
		z -= Math.floor(z);
		...
		// Blends the results from all 8 cube corners
	...
	private double fade(double t) { //finds the fade (smooth transition between gradients)
		return t * t * t * (t * (t * 6 - 15) + 10);
	}
	
	private double lerp(double t, double a, double b) { //finds the linear interpolation
		return a + t * (b - a);
	}
	...
	//finds the gradient for 2D noise functions
